...
class TtfNoGenerate extends Model
{
    // use SoftDeletes;
    protected $connection = 'db_tbs';
    protected $table = 'ttf_entry';
    ...
    public function getStinNo()
    {
        ...
        ->limit('1')
        ->get();
        $stin_sysno = Sys_number::where('label','TTF AR NUMBER')
        ->select('contents')
        ->get();
        ...
        if ($a == $b){ //Jika 4 digit dari sys_number & date sama
            ...
            //kemudian di cek di tabel do hdr
             $cek_stouttbl = DB::connection('db_tbs')->table('ttf_entry')->where('ttf_no',$cek_stinno)
             ->select(['ttf_no'])
             ->get();
